...
- 通过字符线条的密集程度不同可以进行明暗区分，即灰度值区分
  ...
  <img src="../images/paintingByCharacter_1.jpg" style="zoom:50%; float:left" />
...
<img src="../images/paintingByCharacter_2.png" style="zoom:50%; " />
...
- 初始化必要信息，包括但不限于 相关文件输入输出**路径**、**字符序列**
- 提取**灰度**图，调整**尺寸**并获取其**像素矩阵**（必须）~~提取**RGB**图，调整**尺寸**并获取其**像素矩阵**~~（可选）
- 从灰度图中，转出对应字符信息
- 创建画布，将字符信息写到图上
- 保存图片到本地
...
> 虽然没有使用字符序列，但是依然逃不开本文开头 **基本原理** 。
>
> 字符者，大像素也。线条交错，有明暗，可成画矣

优缺点很明显啦，实现简单高容错，但成像却不够细腻
...
> - 视频分解帧，统一文件夹管理
> - 文件夹内，逐帧处理，处理结果输出到新帧文件夹，统一管理
> - 新帧文件夹内读取所有图像文件，组合成新视频
> - ~~选择音频素材，合并音视频~~（可选）
> - ~~播放~~（可选）
...
> 答：黑白字符画的载体可以是文本文件，也可以是图片。因为图像本身、显示器状态、使用的字符序列等因素影响，黑白字符画可以出现 **伪**彩色，即远观多彩放大黑白。彩色字符画一定是以图片为载体，字符序列之于它仅仅是划分明暗，真正色彩是画上字体色深，又回到其底层的像素值上，真正决定了多彩。由此来看，字符的色深是否保持一致也是两种字符画的差异所在。
...
> 答：我想的彩色是指三种即以上的多种色彩同时出现在同一字符画中，所以黑白字符画但是用蓝色字就不是我想要实现的彩色。以此基准，来回答本问题。通过放大**目标 #2**的结果，可以看见因为字体色深确实带来不用颜色，远看画面也比较细腻，但是当看到**文字填充**的结果后，与之相比前者的效果就不够看了，这也是我产生本疑问的来由。实践出真知，换个对比感强的图片，提升很明显，确实实现了彩色。都是彩色字符画，但目前发现细腻和颜色不可兼得。
