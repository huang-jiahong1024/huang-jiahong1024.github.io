...
> 答：我想的彩色是指三种即以上的多种色彩同时出现在同一字符画中，所以黑白字符画但是用蓝色字就不是我想要实现的彩色。以此基准，来回答本问题。通过放大**目标 #2**的结果，可以看见因为字体色深确实带来不用颜色，远看画面也比较细腻，但是当看到**文字填充**的结果后，与之相比前者的效果就不够看了，这也是我产生此疑问的来由。实践出真知，换个对比感强的图片，提升很明显，确实实现了彩色。都是彩色字符画，但目前发现细腻和颜色不可兼得。

字符动画可全自动化实现？

> `os.listdir()` 可忽视总帧数以优化掉 `for i in range(startNo,endNo)`实现自动化，但也因其**无序读取**而导致最终视频**乱帧**。
>
> **解决方法**
>
> ```python
